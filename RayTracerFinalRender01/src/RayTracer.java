...
public class RayTracer {
    //number of samples per pixel for anti-aliasing and maximum recursion depth for reflections and refractions
   ...
   //scales the model by multiplying the vertex coordinates by the given scale factors for each axis
   ...
   public static void main(String[] args) throws Exception {
    //scene setup with camera, objects, materials, and lights, followed by BVH construction and rendering loop with anti-aliasing
    int width  =4096;
    int height = 2160;
       ...
       addWall(scene, new Vector3D(L,-L,Z0), new Vector3D(L, L,Z1), new Vector3D(L, L,Z0), new Color(30,35,50), 0.02,8,0);
        //water model loaded from an .obj file, rotated, scaled, and placed in the scene, with material properties set to make it look like water
       ...
       //pedestals for the objects, with a simple box shape made of triangles and material properties set to make them look like stone
       ...
       //adds the pedestals to the scene by calling the addPedestal method with the appropriate parameters for each pedestals position and size
       addPedestal(scene, cx1, pz1, peW, -L, peY, ped);
       addPedestal(scene, cx2, pz2, peW, -L, peY, ped);
       addPedestal(scene, cx3, pz3, peW, -L, peY, ped);
       //models loaded from .obj files for the shoes, goggles, helmet, and bicycle, with rotations and placements to position them on the pedestals, and material properties set to give them a slightly reflective appearance
       ...
       //scene lighting setup with a dark background color, ambient light, three spotlights positioned above the pedestals to illuminate the objects, and two point lights to add additional illumination and depth to the scene
       ...
       //rendering loop that iterates over each pixel in the output image, generates rays from the camera through the pixel, and traces those rays through the scene to determine the color of each pixel, with anti-aliasing achieved by averaging multiple samples per pixel
    ...
    private static void addWall(Scene sc, Vector3D a, Vector3D b, Vector3D c,Color col, double refl, double shin, double spec) {
       Triangle t = new Triangle(a, b, c, col);
       t.setFullMaterial(refl, shin, spec, 0.0, 1.0);
       sc.addObject(t);
   }    
   //method to create a pedestal by adding triangles to the scene to form a box shape, with the given center position, width, floor and top heights, color, and material properties
    private static void addPedestal(Scene sc, double cx, double pz,double pw, double floorY, double topY, Color c) {
   ...
   //method to center a list of triangles around the origin and then translate them to a specified position in the scene, effectively placing the model at the desired location
   private static Triangle makeTri(double x1,double y1,double z1,double x2,double y2,double z2,
    double x3,double y3,double z3,Color c,double refl,double shin,double spec) {
